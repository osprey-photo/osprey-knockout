...
public class TitlePane extends VBox {
    ...
    public TitlePane(Model model){
		...
		Text chartTitle = new Text(model.getCurrentRound().getRoundTitle());
		...
		getChildren().add(bwps);
		getChildren().add(chartTitle);
		getChildren().add(subTitle);
		
		// Add a 'lightbox' view of all the images in this round
		GridPane grid= new ImageGrid(model);
